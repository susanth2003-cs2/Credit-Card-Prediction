...
def random_sample_imputation_technique(X_train,X_test):
    try:
        logger.info(f'Total Rows in Training_data : {X_train.shape}')
        logger.info(f'Total Rows in Testing_data : {X_test.shape}')
        ...
        for i in X_test.columns:
            if X_test[i].isnull().sum() > 0:
                logger.info(f'Test column name : {i}')
        '''
        for i in X_train.columns:
            if X_train[i].isnull().sum() > 0:
                logger.info(f'Train Column name : {i}')
        ...
        logger.info(f'After Technique X_train : {X_test.isnull().sum()}')
        logger.info(f'Total Rows in Training_data : {X_train.shape}')
        logger.info(f'Total Rows in Testing_data : {X_test.shape}')
